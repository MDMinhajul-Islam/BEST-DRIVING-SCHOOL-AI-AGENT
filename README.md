...
The independent Next.js website is now in [`frontend/`](frontend/). Run
`cd frontend && npm ci && npm run dev`. Voice calling is disabled by default;
it does not simulate calls or expose mock booking data. The optional isolated
FastAPI Retell token broker uses `src.routes.voice:create_app`.
See [frontend setup, environment variables and verification](docs/frontend.md)
for running both apps, enabling real Retell calls and deployment boundaries.
