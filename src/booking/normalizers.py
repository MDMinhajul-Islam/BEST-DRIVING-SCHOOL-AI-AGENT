...
def result(mode,**fields):return {'success':True,'mode':mode,'booking_provider':'mock' if mode=='mock' else 'calcom','production':False,'synthetic':mode=='mock','demo':True,'booking_confirmed':False,**fields}
def error(code,mode):
 ...
 return {'success':False,'mode':mode,'booking_provider':'mock' if mode=='mock' else 'calcom','production':False,'demo':True,'booking_confirmed':False,'error_code':code,'user_safe_message':messages.get(code,'The scheduling request could not be completed safely.')}
